...
local FormatGroup = function(song)
    if song then
        local fmt = "%s: %s"
        return fmt:format(THEME:GetString("ScreenGameplay", "Pack"), song:GetGroupName())
    end
    return ""
end
...
local FormatSongArtist = function(song)
    if song then
        local fmt = "%s: %s"
        return fmt:format(THEME:GetString("ScreenGameplay", "Artist"), song:GetDisplayArtist())
    end
    return ""
end
...
af[#af + 1] = LoadFont("Common Normal") .. {
    ...
    MarqueeCommand = function(self)
        ...
        self:settext(fmt:format(THEME:GetString("ScreenGameplay", "Steps"), text))
